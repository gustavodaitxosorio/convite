...
- **Botão "não":** brincadeira nova — ao passar o mouse (ou tocar), **treme e vira "Sim"**
  (texto muda, cor vira rosa). Impossível recusar. NÃO é o "foge do cursor" da v2.
...
4. **Tela Pergunta** (`#question`) — efeito máquina de escrever monta a pergunta;
   botões **Sim** e **não**; gag do "não" → "Sim". Primeiro clique dispara a música.
